...
uses UnDm, UnDmFluxo, UnRelFluxo, UnPrincipal;
...
begin
FrmFluxo.Caption:= FrmFluxo.Caption;
Exit;
end else
FrmFluxo.Caption:= FrmFluxo.Caption+' - '+Dm.IBDS_EMPRESANOME_FANTASIA.AsString;
PageControl.SetFocus;
FrmFluxo.Caption:= TabSheet0.Caption;
// abre tabelas
if DmFluxo.ds.DataSet <> nil then
DmFluxo.ds.dataset.active := false;
begin
        with DmFluxo.QueryFluxo do
        begin
          Close;
        ...
        DmFluxo.Provider.DataSet := DmFluxo.QueryFluxo;
        DmFluxo.DS.Dataset := DmFluxo.cdsFluxo;
      end;
...
procedure TFrmFluxo.BtnRefreshClick(Sender: TObject);
begin
// INFORMA O FLUXO DO CAIXA
Recebimentos;
if BtnRefresh.Caption = TabSheet0.Caption then
begin
DmFluxo.cdsFluxo.disableControls;
  try
    DmFluxo.cdsFluxo.active := false;
    with DmFluxo.QueryFluxo do
    begin
      Close;
      ParamByName('dia1').AsDate:= Dia1.date;
    ...
    DmFluxo.cdsFluxo.active := true;
  finally
    DmFluxo.cdsFluxo.enableControls;
  end;
   DmFluxo.cdsFluxo.Last;
  Exit;
end else
...
procedure TFrmFluxo.BtnFecharClick(Sender: TObject);
begin
Close;
end;

procedure TFrmFluxo.DBGrid_FluxoDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
// PINTA O GRID DO FLUXO DO CAIXA
if DBGrid_Fluxo.DataSource = DmFluxo.DS then
begin
if DmFluxo.DS.Dataset = DmFluxo.cdsFluxo then
begin
if not odd(DmFluxo.cdsFluxo.RecNo) then // se for impar
...
if Column.Field = DmFluxo.cdsFluxoVALOR then

if DmFluxo.cdsFluxoVALOR.AsCurrency < -0 then
begin
...
if Column.Field = DmFluxo.cdsFluxoHISTORICO then
if (DmFluxo.cdsFluxoHISTORICO.AsString = 'SALDO ANTERIOR') or (DmFluxo.cdsFluxoHISTORICO.AsString = 'SALDO FINAL') then
begin
DBGrid_Fluxo.Canvas.Font.Style:=
DBGrid_Fluxo.Canvas.Font.Style + [fsBold];
...
procedure TFrmFluxo.BtnImprimirClick(Sender: TObject);
begin
// INFORMA O FLUXO DO CARNÊ
Screen.Cursor := crHourGlass;
// INICIA SELEÇÃO
...
if FrmRelFluxo <> Nil then Exit;
FrmRelFluxo := TFrmRelFluxo.Create(self);
FrmRelFluxo.QuickRep1.OnPreview := FrmPrincipal.CustomPreview;
FrmRelFluxo.QuickRep1.Preview;
FrmRelFluxo.Free;
FrmRelFluxo := Nil;
end;

procedure TFrmFluxo.PageControlChange(Sender: TObject);
begin
case PageControl.TabIndex of
    0: begin // fluxo
    BtnRefresh.Caption:= PageControl.ActivePage.Caption;
    DBGrid_Fluxo.DataSource:= DmFluxo.DS;
    end;
    1: begin // carnê
    BtnRefresh.Caption:= PageControl.ActivePage.Caption;
...
procedure TFrmFluxo.ComboBoxVendedorKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFrmFluxo.DBGrid_FluxoDrawDataCell(Sender: TObject;
...
Valor:= DmFluxo.cdsFluxoVALOR.AsString;
Label_SaldoCaixa.Caption:= FormatFloat('R$ #,##0.00', StrToFloat(Valor));
end;

procedure TFrmFluxo.Timer1Timer(Sender: TObject);
begin
Dia1.Time:= Time;
Dia2.Time:= Time;
end;

procedure TFrmFluxo.Recebimentos;
begin
