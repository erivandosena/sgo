...
var
  FrmRelReciboVA: TFrmRelReciboVA;
...
procedure TFrmRelReciboVA.DataSource1DataChange(Sender: TObject;
  Field: TField);
var
  MS: TMemoryStream;
  J1: TJPEGImage;
...
procedure TFrmRelReciboVA.ColumnHeaderBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
QRLabel_Valor.Caption:= FloatToStrF(SubTotal,ffFixed,18,2);
QRMemo_Extenco.Lines.Text:= 'A importância de   '+Extenso(StrToFloat(QRLabel_Valor.Caption),aMoeda);
QRMemo_Referente.Lines.Text:= 'referente a pagamento Á VISTA no '+FormaVenda+', sob contrato Nº: '+NC+'.';
...
procedure TFrmRelReciboVA.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
